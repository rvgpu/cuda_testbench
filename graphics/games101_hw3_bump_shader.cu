...
__device__ Eigen::Vector3f bump_shader_color(
...
__global__ void fragment_shader(
    ...
    if (pixel_in_triangle) {
        ...
        if (z > out_depth_buffer[pixel_id]) {
            ...
            Eigen::Vector3f color = bump_shader_color(interpolated_normal, in_tex_buffer, in_tex, interpolated_texcoords);
...
int main() {
    ...
    write_ppm("games101_hw3_bump_shader", WIDTH, HEIGHT, image);
}