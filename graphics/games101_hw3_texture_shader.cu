...
__device__ Eigen::Vector3f texture_shader_color(
    Eigen::Vector3f viewspace_normal,
    uint8_t *tex_buffer,
    struct tex_info *tex,
...
__global__ void fragment_shader(
    ...
    if (pixel_in_triangle) {
        ...
        if (z > out_depth_buffer[pixel_id]) {
            ...
            Eigen::Vector3f color = texture_shader_color(interpolated_normal, in_tex_buffer, in_tex, interpolated_texcoords, interpolated_pos);
...
int main() {
    ...
    write_ppm("games101_hw3_texture_shader", WIDTH, HEIGHT, image);
}