...
__device__ Eigen::Vector3f get_tex_color(uint8_t *tex_buffer, float tex_u, float tex_v, int w, int h) {
...
__device__ struct displacement bump_shader_modify(
    struct displacement dp,
    uint8_t *tex_buffer,
    struct tex_info *tex,
    ...
    Eigen::Vector3f point = dp.point;

    // Normal
    Eigen::Vector3f normal = dp.normal;
    float x = normal.x();
    float y = normal.y();
    float z = normal.z();
    ...
    struct displacement modified_dp;

    modified_dp.point = point + kn * normal * color0.norm();
    modified_dp.normal = (TBN * ln).normalized();
...
__device__ Eigen::Vector3f displacement_shader_color(
    Eigen::Vector3f color,
    Eigen::Vector3f viewspace_normal,
    uint8_t *tex_buffer,
    struct tex_info *tex,
    ...
    struct displacement dp = {viewspace_pos, viewspace_normal};
    
    // Use bump shader to modify the point and the normal
    dp = bump_shader_modify(dp, tex_buffer, tex, tex_coords);

    // Use phong shader to compute the color
    result_color = phong_shader_color(color, dp.normal, dp.point);    
...
__global__ void fragment_shader(
    ...
    uint8_t *in_tex_buffer,
    struct tex_info *in_tex,
    ...
    if (pixel_in_triangle) {
        ...
        if (z > out_depth_buffer[pixel_id]) {
            ...
            Eigen::Vector3f color = displacement_shader_color(
                interpolated_color,
                interpolated_normal,
                in_tex_buffer,
                in_tex,
            ...
            out_color_buffer[pixel_id * 4 + 0] = color_r;
            out_color_buffer[pixel_id * 4 + 1] = color_g;
            out_color_buffer[pixel_id * 4 + 2] = color_b;
...
int main() {
    ...
    // Texture buffer
    uint8_t *fs_in_tex_buffer;
    cudaMalloc(&fs_in_tex_buffer, tex_width * tex_height * 4 * sizeof(uint8_t));
    cudaMemcpy(fs_in_tex_buffer, tex_buffer, tex_width * tex_height * 4 * sizeof(uint8_t), cudaMemcpyHostToDevice);
    ...
    struct tex_info *fs_in_tex;
    cudaMalloc(&fs_in_tex, sizeof(struct tex_info));
    cudaMemcpy(fs_in_tex, &tex, sizeof(struct tex_info), cudaMemcpyHostToDevice);
    ...
    for (int i = 0; i < triangle_num; i++) {
        ...
        fragment_shader<<<num_blocks, threads_per_block>>>(fs_in_triangle, fs_in_box, fs_in_tex_buffer, fs_in_tex, fs_in_viewspace_pos, fs_out_depth_buffer, fs_out_color_buffer);
    ...
    write_ppm("games101_hw3_displacement_shader", WIDTH, HEIGHT, image);
}